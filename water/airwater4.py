...
def watermark(base: Image.Image, opacity=0.7, font_size=16, left=12, right=35): #100

    overlay = Image.new("RGBA", base.size, (0, 0, 0, 0))
    draw = ImageDraw.Draw(overlay)

    # try:
    font = ImageFont.truetype("public2/css/font/GB.ttf", font_size)
    # except IOError:
        # font = ImageFont.load_default()

    r, g, b = (60, 60, 90)
    alpha = int(255 * max(0, min(1, opacity)))
    draw.text((left, right), 'SK™Automobile', fill=(r, g, b, alpha), font=font)
    #125,250
...
async def translate_text(T):
    async with Translator() as translator:
        result = await translator.translate(T)
        print(result.text)
        return result.text
...
async def process_item(client, item):
    ...
    try:
        ...
        return {
            ...
            "image_data2": watermark(img2, left=170, right=150)
